...
func TestVerifyChecksum(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:        "valid checksum",
			checksumTxt: fmt.Sprintf("%s  codeagent-sync-darwin-arm64\n", correctHash),
			assetName:   "codeagent-sync-darwin-arm64",
			data:        binaryData,
			wantErr:     false,
		},
		{
			name:        "valid checksum with binary mode marker",
			checksumTxt: fmt.Sprintf("%s *codeagent-sync-linux-x64\n", correctHash),
			assetName:   "codeagent-sync-linux-x64",
			data:        binaryData,
			wantErr:     false,
		},
		{
		...
		},
		{
			...
			checksumTxt: fmt.Sprintf("%s  codeagent-sync-darwin-arm64\n", correctHash),
			assetName:   "codeagent-sync-linux-arm64",
			data:        binaryData,
			wantErr:     true,
		...
		},
		{
			...
			checksumTxt: fmt.Sprintf(`%s  codeagent-sync-darwin-arm64
abc123  codeagent-sync-darwin-x64
def456  codeagent-sync-linux-arm64
`, correctHash),
			assetName: "codeagent-sync-darwin-arm64",
			data:      binaryData,
			wantErr:   false,
...
func TestVerifyChecksumNoChecksumsFile(t *testing.T) {
	release := &GitHubRelease{
		Assets: []struct {
			...
			{
				Name:               "codeagent-sync-darwin-arm64",
	...
	err := verifyChecksum(release, "codeagent-sync-darwin-arm64", []byte("data"))
	if err != nil {
