...
type parsedResponse struct {
	Href          string // decoded path, for display
	RawHref       string // href as the server sent it, percent-encoded
	ContentLength int64
...
func parsePropfindResponse(data []byte) ([]parsedResponse, error) {
	...
	for _, entry := range ms.Responses {
		...
		href := strings.TrimSpace(entry.Href)
		r.RawHref = href
		...
		for _, ps := range entry.Propstat {
			...
			r.ETag = normalizeETag(ps.Prop.ETag)
