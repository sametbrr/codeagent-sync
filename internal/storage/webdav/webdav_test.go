...
func TestClientURLBuilding(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wantFullURL:    "https://cloud.example.com/dav/codeagent-sync/sessions/abc.age",
			wantCollection: "https://cloud.example.com/dav/codeagent-sync/",
		},
		{
...
func TestPutSendsAuthAndContentType(t *testing.T) {
	...
	if _, err := client.Put(context.Background(), "sessions/test.age", []byte("encrypted data"), storage.Precondition{}); err != nil {
		t.Errorf("Put() error = %v", err)
	}
}

func TestPutReportsHTTPErrors(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method == "MKCOL" {
			w.WriteHeader(http.StatusCreated)
			return
	...
	_, err := client.Put(context.Background(), "test.age", []byte("data"), storage.Precondition{})
	if err == nil || !strings.Contains(err.Error(), "HTTP 500") {
		t.Errorf("Put() error = %v, want one naming HTTP 500", err)
	}
}
...
func TestGet(t *testing.T) {
	...
	data, _, err := client.Get(context.Background(), "test.age")
	if err != nil {
		t.Errorf("Get() error = %v", err)
	}
	if string(data) != string(expectedData) {
		t.Errorf("Get() = %q, want %q", string(data), string(expectedData))
	}
}
...
func TestGetNotFound(t *testing.T) {
	...
	if _, _, err := client.Get(context.Background(), "nonexistent.age"); !errors.Is(err, storage.ErrNotFound) {
...
<d:multistatus xmlns:d="DAV:">
  <d:response>
	...
	if info.ETag != `"etag456"` {
		t.Errorf("Head() ETag = %q, want %q", info.ETag, `"etag456"`)
	}
}
...
func TestBucketExistsAutoCreate(t *testing.T) {
	...
	client := &Client{
		baseURL:    server.URL,
		pathPrefix: "codeagent-sync", // with prefix, 404 triggers auto-create
		username:   "user",
		password:   "pass",
		httpClient: server.Client(),
	}
...
func TestListDepthInfinityFallback(t *testing.T) {
	const prefix = "codeagent-sync"

	// Tree under /codeagent-sync/:
	//   settings.enc
	//   sessions/a.enc
	//   sessions/nested/b.enc
	tree := map[string]string{
		"/codeagent-sync/":                 multistatusFor("/codeagent-sync/", []string{"settings.enc"}, []string{"sessions"}),
		"/codeagent-sync/sessions/":        multistatusFor("/codeagent-sync/sessions/", []string{"a.enc"}, []string{"nested"}),
		"/codeagent-sync/sessions/nested/": multistatusFor("/codeagent-sync/sessions/nested/", []string{"b.enc"}, nil),
	}
...
func TestListDepthInfinitySuccess(t *testing.T) {
	...
	full := `<?xml version="1.0"?><d:multistatus xmlns:d="DAV:">` +
		`<d:response><d:href>/codeagent-sync/</d:href><d:propstat><d:prop>` +
		`<d:resourcetype><d:collection/></d:resourcetype></d:prop><d:status>HTTP/1.1 200 OK</d:status></d:propstat></d:response>` +
		`<d:response><d:href>/codeagent-sync/settings.enc</d:href><d:propstat><d:prop>` +
		`<d:resourcetype/><d:getcontentlength>10</d:getcontentlength>` +
		`<d:getlastmodified>Mon, 14 Apr 2025 10:00:00 GMT</d:getlastmodified><d:getetag>"x"</d:getetag></d:prop>` +
		`<d:status>HTTP/1.1 200 OK</d:status></d:propstat></d:response>` +
		`<d:response><d:href>/codeagent-sync/sessions/a.enc</d:href><d:propstat><d:prop>` +
		`<d:resourcetype/><d:getcontentlength>10</d:getcontentlength>` +
